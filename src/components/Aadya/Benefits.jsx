...
const Benefits = () => {
  return (
    <Section id="features">
      <div className="flex justify-center items-center relative z-2 mt-14 md:mt-0">
      ...
      <div className="container grid lg:grid-cols-3 gap-5 md:gap-10 lg:gap-20 mt-6 lg:mt-10">
        {benefits.map((items,index) => (
          <div key={index} className="text-black bg-gray-200 p-10 items-center rounded-2xl">
            <h1 className="text-2xl font-semibold text-center">{items.title}</h1>
            <p className="mt-5 px-5 text-lg">{items.text}</p>
          </div>
        ))}
        <div>
        </div>
